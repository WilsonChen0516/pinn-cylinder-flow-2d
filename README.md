...
| 實驗 | 目標 | 主要指標 | 結果 |
|:-----|:-----|:---------|:-----|
| **1. Inverse Problem** | 稀疏觀測反推流場 + 識別 λ₁, λ₂ | λ₁ 相對誤差 | **0.054%** ✅ |
|  |  | λ₂ 相對誤差 | **3.17%** ✅ |
|  |  | L2(u) | 0.34% |
| **2. Data Efficiency** | 觀測量對精度的影響 | PINN vs. MLP 誤差比 | **PINN 領先 5×** |
| **3. Forward Problem** | 僅用 IC + BC 求解流場 | L2(u) | 1.01% |
|  |  | L2(v) | 3.90% |

✅ **Inverse problem 的 λ 識別精度全面超越 Raissi 2019 原論文**（0.078% / 4.67%）。
